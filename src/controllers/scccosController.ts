 ...
 */
const renderAndStoreScccosPdf = async (req: Request, id: string): Promise<RenderedPdf | null> => {
  ...
  const buffer = await createScccosPdfBuffer(certificate, qrBuffer);
...
const invalidateLinkedSurveyReportPdf = async (surveyReportId: unknown): Promise<void> => {
  ...
  await SurveyReportModel.updateOne(
    { firstEntrySurveyReportId: surveyReportId },
    { $unset: { pdf: 1 } },
    { timestamps: false }
  );
};
